...
Если ваши компоненты содержат `<router-link>`, в примерах Styleguidist лучше подменить его упрощенным компонентом. Для этого добавьте файл `styleguide.global.requires.js` (пример ниже) и подключите его через опцию [require](/Configuration.md#require). Не подключайте `vue-router` напрямую внутри Styleguidist.
